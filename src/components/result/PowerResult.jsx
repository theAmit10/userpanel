...
function PowerResult({ reloadKey, setSelectedCategory }) {
  ...
  const {
    ...
    {
      accesstoken,
      powertimeid: selectedItem?._id,
      year: selectedYear,
      month: selectedMonth,
    },
    { skip: selectedItem == null }
  );
  ...
  useEffect(() => {
    if (!allocationIsLoading && alllocation) {
      ...
      console.log("For Time :: ", alllocation?.powerTimes[0]?.powertime);
    }
  ...
  function convertDatesToTimezone(dataObject, targetTimezone) {
    return {
      ...
      results: dataObject.results.map((resultItem) => {
        const powertime = resultItem.powertime.powertime; // e.g., "02:35 AM"

        return {
          ...resultItem,
          dates: resultItem.dates.map((dateObj) => {
            const powerdate = dateObj.powerdate.powerdate; // e.g., "21-10-2024"

            // Combine powerdate and powertime to create a complete DateTime in IST
            ...
            // Return a new dateObj with the updated powerdate and powertime
            return {
              ...dateObj,
              powerdate: {
                ...dateObj.powerdate,
                powerdate: targetDateTime.format("DD-MM-YYYY"), // Update powerdate
                powertime: targetDateTime.format("hh:mm A"), // Update powertime
              },
            };
          }),
        };
  ...
  const transformData = (data) => {
    ...
    const rows = data[0].dates.map((dateItem) => {
      const row = [dateItem.powerdate.powerdate];
      data.forEach((item) => {
        const dateData = item.dates.find(
          (d) => d.powerdate.powerdate === dateItem.powerdate.powerdate
        );
        row.push(
          dateData
            ? dateData.results[0]?.jackpotnumber
                .toString()
                .replaceAll(",", " ") || "-"
            : "-"
        );
        ...
        <div className="alContainer">
          ...
          {allocationIsLoading ? (
            ...
            <div className="PLContainerMain">
              <div className="ARLCC">
                {alllocation?.powerTimes?.map((item, index) => (
                  <div
                    key={index}
                  ...
                  >
                    ...
                    >
                      <label className="locLabel">{item.powertime}</label>
                            ...
                            {allresult?.results?.map((item, resultIndex) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {matchingDate?.results?.[0]?.jackpotnumber
                                    .toString()
                                    .replaceAll(",", " ") || "-"}
                                </td>
                              );
